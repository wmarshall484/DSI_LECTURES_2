...
\connect readychef -- database created during individual sprint
...
--self join on above query showing all meals that are above the average price and list the afterage price for that catergory next to it
...
--- same as above but using WHERE instead of ON/AND and limit of 10 (and discarding some columns)
...
SELECT COUNT(*), type
	...
	WHERE price > (SELECT AVG(price) FROM meals)
	GROUP BY type;
--subquery results in a single number and canbe used as such!
...
-- same without HAVING
-- using WITH (creates a sort of temp table)
