...
def plot_betas(asbs, save_fig=False):
    ...
    #ax.legend(labels=['A','B'], prop={'size':30})
    ax.set_xlabel('p', fontsize=24)
    ax.set_ylabel(r'$Beta(p;\alpha, \beta)$', fontsize=24)
    #ax.set_ylabel('probability density function', fontsize=24)
    ax.set_xlim([0,1.000])    
    ax.tick_params(axis='both', which='major', labelsize=20)
    fig.suptitle('Beta Distribution', fontsize=30)
    #fig.suptitle('Posterior distributions for click-through rates', fontsize=30)    

    if save_fig:
        plt.savefig('betas_cred.png')
    else:
        plt.show()
...
#    plot_betas([(1,1),(3,3),(10,5),(50,20),(100,200)], save_fig=True)
#    plot_betas([(15,17),(9,23)], save_fig=True)
    plot_betas([(4,22)], save_fig=True)
